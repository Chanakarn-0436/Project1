...
class SupabaseManager:
    ...
    def __init__(self):
        ...
        self.storage_bucket: str = st.secrets.get("STORAGE_BUCKET") or os.getenv("STORAGE_BUCKET") or "uploads"
    ...
    def upload_to_storage(self, file_bytes: bytes, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            # อัปโหลดไฟล์โดยตรงไปที่ bucket (ไม่ต้องเช็คหรือสร้าง)
            ...
            if hasattr(result, 'path') and result.path:
                ...
                # Upload ไม่สำเร็จ
                print("❌ Upload failed - no path returned")
                return None
    ...
    def get_storage_base_url(self) -> str:
        """ดึง base URL ของ Supabase Storage"""
        if not self.is_connected():
            return ""
        
        try:
            # ดึง URL จาก Supabase client
            url = st.secrets.get("SUPABASE_URL") or os.getenv("SUPABASE_URL")
            if url:
                # แปลงจาก https://xxx.supabase.co เป็น https://xxx.supabase.co/storage/v1/object/public
                return url.replace("supabase.co", "supabase.co/storage/v1/object/public")
            return ""
        except Exception as e:
            print(f"❌ Failed to get storage base URL: {e}")
            return ""
