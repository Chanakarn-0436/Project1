...
class _SiteBucket:
    ...
    # (traffic_hex, conn_hex, state, raw_line, source_hex, dest_hex)
...
class ApoRemnantAnalyzer:
    def __init__(self, raw_text: str, site_map: Dict[str, str] | None = None):
        ...
        # APO remnant links: {(source_name, dest_name): count}
        ...
        for ln in self.lines:
            ...
            if cap_apop and ln.startswith("[APOPLUS]"):
                ...
                if mrow:
                    ...
                    state      = mrow.group(5)
                    self.per_site[cur_apop_site_ip].apop_rows.append((traffic, connno, state, ln, source_hex, dest_hex))
                continue
            ...
            # Case 2️⃣ APOP เกิน → แดงเฉพาะฝั่ง APOP
                ...
                for p in extra:
                    ...
                    if data:
                        ...
                        # เก็บข้อมูล source→destination link
...
def apo_kpi(rendered: list[tuple], apo_links: dict = None):
    """
    rendered: list ที่ได้จาก ApoRemnantAnalyzer.analyze()
    apo_links: dict of (source_name, dest_name) -> count
    """
    # ---------- Summary Count ----------
    ...
    # ---------- APO Links Summary ----------
    if apo_links and apo_sites > 0:
        st.markdown("#### APO Remnant Links")
        link_summary = []
        for (src, dst), count in sorted(apo_links.items()):
            link_summary.append(f"**{src} → {dst}** ({count} remnant{'s' if count > 1 else ''})")
        st.markdown(" • ".join(link_summary))
