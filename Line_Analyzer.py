...
class Line_Analyzer:
    ...
    def process(self) -> None:
        ...
        # ✅ แสดง Problem Call IDs เฉพาะ BER abnormal ที่นี่
        self._render_ber_only_abnormal_data(df_filtered)
    ...
    def _render_ber_only_abnormal_data(self, df_view: pd.DataFrame) -> None:
        """แสดง Problem Call IDs เฉพาะ BER abnormal เท่านั้น"""
        ...
        # ✅ ใช้เฉพาะ BER abnormal เท่านั้น (ไม่รวม Input/Output)
        mask_any_abnormal = mask_ber
        
        # ✅ เลือกคอลัมน์ที่จำเป็นเฉพาะ BER เท่านั้น
        ...
        # ✅ แปลงค่าเป็น numeric ก่อน format เฉพาะ BER
        ...
        st.markdown(f"**Problem Call IDs (BER abnormal)** - Found {len(fail_rows)} rows")
    ...
    def _render_summary_kpi(self, df_view: pd.DataFrame) -> None:
        ...
        # ✅ ใช้เงื่อนไขเดียวกับ prepare() และ _render_ber_only_abnormal_data()
        # BER abnormal = (ber > 0 หรือ ber เป็น None) และ threshold ไม่เป็น None
        mask_ber_abnormal = ((ber > 0) | ber.isna()) & thr.notna()
        fail_ber_cnt = int(mask_ber_abnormal.sum())
        
        # BER normal = threshold เป็น None หรือ (ber <= threshold และ ber ไม่เป็น None)
        mask_ber_normal = thr.isna() | ((ber <= thr) & ber.notna())
        ok_ber_cnt = int(mask_ber_normal.sum())
        ...
        # ✅ ใช้เงื่อนไขเดียวกับ prepare()
        # Input abnormal = มีค่า input และ threshold และอยู่นอกช่วง
        mask_input_abnormal = (vin.notna() & min_in.notna() & max_in.notna() & ((vin < min_in) | (vin > max_in)))
        fail_in_cnt = int(mask_input_abnormal.sum())
        
        # Input normal = ไม่มีค่า input หรือ threshold หรืออยู่ในช่วง
        mask_input_normal = vin.isna() | min_in.isna() | max_in.isna() | ((vin >= min_in) & (vin <= max_in))
        ok_in_cnt = int(mask_input_normal.sum())
        ...
        # ✅ ใช้เงื่อนไขเดียวกับ prepare()
        # Output abnormal = มีค่า output และ threshold และอยู่นอกช่วง
        mask_output_abnormal = (vout.notna() & min_out.notna() & max_out.notna() & ((vout < min_out) | (vout > max_out)))
        fail_out_cnt = int(mask_output_abnormal.sum())
        
        # Output normal = ไม่มีค่า output หรือ threshold หรืออยู่ในช่วง
        mask_output_normal = vout.isna() | min_out.isna() | max_out.isna() | ((vout >= min_out) & (vout <= max_out))
        ok_out_cnt = int(mask_output_normal.sum())
    ...
    def _render_line_charts(self, df_view: pd.DataFrame) -> None:
        ...
        # ไม่แสดง Problem Call IDs ในส่วนนี้ - ให้แสดงเฉพาะกราฟ
        ...
        def _plot_board(df_board_raw: pd.DataFrame, board_name: str):
            ...
            if not df_problems.empty:
                ...
                # --- ใช้สไตล์ที่เน้นสีแดงเฉพาะ Input/Output abnormal ---
                styled = (
                    df_display.style
                    # 🌑 ไฮไลต์ gray ถ้ามีปัญหา (Input/Output เท่านั้น)
                    .apply(lambda r: [
                        'background-color:#e6e6e6; color:black' if self._row_has_issue(
                            r, "Instant BER After FEC",
                            self.col_out, self.col_max_out, self.col_min_out,
                            self.col_in,  self.col_max_in,  self.col_min_in
                        ) else ''
                        for _ in r
                    ], axis=1)
                    # 🔴 ไฮไลต์ Output abnormal (ไม่รวม BER)
                    .apply(lambda _: [
                        'background-color:#ff4d4d; color:white'
                        if (pd.notna(v) and pd.notna(hi) and pd.notna(lo) and (v > hi or v < lo)) else ''
                        for v, hi, lo in zip(
                            df_display.get(self.col_out, []),
                            df_display.get(self.col_max_out, []),
                            df_display.get(self.col_min_out, []),
                        )
                    ], subset=[self.col_out] if self.col_out in df_display.columns else [])
                    # 🔴 ไฮไลต์ Input abnormal (ไม่รวม BER)
                    .apply(lambda _: [
                        'background-color:#ff4d4d; color:white'
                        if (pd.notna(v) and pd.notna(hi) and pd.notna(lo) and (v > hi or v < lo)) else ''
                        for v, hi, lo in zip(
                            df_display.get(self.col_in, []),
                            df_display.get(self.col_max_in, []),
                            df_display.get(self.col_min_in, []),
                        )
                    ], subset=[self.col_in] if self.col_in in df_display.columns else [])
                    # 🔵 ไฮไลต์ Route ที่เป็น Preset
                    .apply(lambda _: [
                        'background-color:lightblue; color:black'
                        if str(x).startswith("Preset") else ''
                        for x in df_display.get("Route", [])
                    ], subset=["Route"] if "Route" in df_display.columns else [])
                    # ✅ กำหนดรูปแบบการแสดงผล
                    .format({
                        self.col_out: "{:.4f}", 
                        self.col_in: "{:.4f}",
                        ...
                        self.col_max_in: "{:.4f}",  
                        self.col_min_in: "{:.4f}",
                        "Instant BER After FEC": "{:.2E}",   # ไม่เน้นสีแดง
                        "Threshold": "{:.2E}",               # ไม่เน้นสีแดง
                    }, na_rep="None")
                )
                st.dataframe(styled, use_container_width=True)
