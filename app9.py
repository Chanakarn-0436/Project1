...
def save_file(upload_date: str, file, use_storage: bool = True):
    ...
    if use_storage and supabase.is_connected():
        # อัปโหลดไป Supabase Storage โดยตรง (ไม่เช็ค bucket)
        ...
        if storage_url:
            ...
            print("⚠️ Supabase Storage upload failed, falling back to local storage")
            use_storage = False
...
def delete_file(file_id: int):
    ...
    # ดึงข้อมูลไฟล์จาก Supabase
    files = supabase.supabase.table("uploads").select("stored_path").eq("id", file_id).execute() if supabase.is_connected() else None
    
    if files and files.data:
        stored_path = files.data[0]["stored_path"]
        try:
            os.remove(stored_path)  # ลบไฟล์จากดิสก์
        except FileNotFoundError:
            pass
    
    # ลบจาก Supabase
    supabase.delete_file_record(file_id)

def list_dates_with_files():
    """ดึงรายการวันที่ที่มีไฟล์จาก Supabase"""
    return supabase.get_dates_with_files()
...
KW = {
    ...
    # ✅ ลบ preset ออกจาก KW เพราะใช้ wason แทน
}
...
def _kind(name):
    ...
    # ---- Priority ----
    # ✅ ไฟล์ txt ที่มี "wason" จะใช้สำหรับ Preset และ APO analysis
    if "wason" in hits:
        return "wason"
...
if original_menu == "Home":
            ...
            if not selected_files:
                ...
                if st.session_state.get("cpu_data") is not None:
                    try:
                        ...
                        df_abnormal = getattr(analyzer, "df_abnormal", pd.DataFrame())
                        st.session_state["cpu_abn_count"] = len(df_abnormal) if not df_abnormal.empty else 0
                        st.session_state["cpu_status"] = "Abnormal" if st.session_state["cpu_abn_count"] > 0 else "Normal"
                    except Exception as e:
                ...
                if (st.session_state.get("osc_data") is not None and 
                    ...
                    try:
                        ...
                        # Set session state for sidebar indicator
                        df_abnormal = getattr(analyzer, "df_abnormal", pd.DataFrame())
                        abn_count = len(df_abnormal) if not df_abnormal.empty else 0
                        st.session_state["fiberflapping_abn_count"] = abn_count
                        st.session_state["fiberflapping_status"] = "Abnormal" if abn_count > 0 else "Normal"
                    except Exception as e:
    ...
    if supabase.is_connected():
        ...
        # ✅ Ensure Preset analyzer is ready if WASON log already loaded
        try:
            if (st.session_state.get("preset_analyzer") is None and 
                st.session_state.get("wason_log") is not None):
                analyzer = PresetStatusAnalyzer(st.session_state["wason_log"])
                analyzer.parse(); analyzer.analyze()
                st.session_state["preset_analyzer"] = analyzer
        except Exception as e:
            st.write(f"Preset analyzer init failed: {e}")
        ...
        try:
            if st.session_state.get("line_data") is not None:
                ...
                # BER abnormal rule aligned with Line_Analyzer: (BER > 0 or BER is None) and Threshold is not None
                ber_abn = int((((ber > 0) | ber.isna()) & thr.notna()).sum())
                ...
                # (Removed) Preset subsection under Line to avoid duplication on Dashboard
            else:
        ...
        try:
            ...
            if not preset_analyzer and wason:
                # lazy init
                analyzer = PresetStatusAnalyzer(wason)
                analyzer.parse(); analyzer.analyze()
                st.session_state["preset_analyzer"] = analyzer
                preset_analyzer = analyzer
            
            if preset_analyzer:
                df, summary = preset_analyzer.to_dataframe()
                total = int(summary.get("total", 0))
                passes = int(summary.get("passes", 0))
                fails  = int(summary.get("fails", 0))
                c = st.columns(3)
                c[0].metric("Preset Success", f"{passes}")
                c[1].metric("Preset Abnormal", f"{fails}")
                c[2].metric("Total Preset Calls", f"{total}")
            else:
                st.info("Upload WASON log and run analysis to populate Preset dashboard.")
        except Exception as e:
            st.warning(f"Preset dashboard could not be rendered: {e}")
    ...
    if st.session_state.get("wason_log") is not None:
        try:
            ...
            # ✅ แจ้งเตือนสั้น ๆ ว่ามีสรุปใน Summary table ด้วย
            st.info("ℹ️ Results are also summarized in 'Summary table & report'.")
