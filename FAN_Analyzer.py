...
class FAN_Analyzer:
    ...
    def process(self) -> pd.DataFrame:
        ...
        for ftype, th in thresholds.items():
            ...
            # ตรวจสอบสถานะ Normal/Abnormal (แสดงเฉพาะ Warning เท่านั้น)
            has_abnormal = (df_sub["Avg Fan Speed (Rps)"] > th).any()
            if has_abnormal:
                st.markdown(
                    "<div style='text-align:center; font-size:18px; font-weight:bold; color:red; margin-bottom:10px;'>FAN Performance Warning</div>",
                    unsafe_allow_html=True
                )
